...
Write this at full engineering depth (`AGENTS.md` §10): file:line references,
...
```bash
cd "D:/res/git/MoRE/.Codex/worktrees/english-language-dataset-migration-92946e" && git add TASKS_LANGUAGE.md changelog.md <the files the task touched> && git status --short
```

Commit message: `<task id>: <what changed>`, then a body naming the verification.
End with the `Co-Authored-By: Codex <noreply@anthropic.com>` trailer.
...
- Do not rewrite history in `changelog.md`, `TASKS.md`, `results/results.md` or
  ...
  corrections go in the *new* entry and reference the old one. `AGENTS.md` §6:
