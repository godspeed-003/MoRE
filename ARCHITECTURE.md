...
Both algorithms live in [code/more/metrics.py](code/more/metrics.py) and are
checked against brute force rather than against a library. They are local because
they are *exact* at `E <= 15` and because the metric layer then imports nothing
beyond torch/numpy — not because scipy/sklearn are unavailable. (An earlier
version of this paragraph said `more_env` had neither; sklearn 1.3.2 and scipy
1.14.1 are both importable here. Corrected in T-L0.0.)
...
Full record, including how the CUDA venv was built and why: [ENVIRONMENT.md](ENVIRONMENT.md).

```
C:\Users\vedan\anaconda3\python.exe                 torch 2.6.0+cpu    CPU, gates
D:\res\git\MoRE\.venv_cuda\Scripts\python.exe       torch 2.6.0+cu126  CUDA, training
```

GPU is an **RTX 3050 6 GB Laptop** (6143 MiB, compute capability 8.6, driver
596.08). Base anaconda's torch is CPU-only, so training must use the venv;
correctness gates are CPU work and use base. Shell is Git Bash on Windows; the
working directory persists between calls, so **use absolute paths**. The console
is cp1252 — Unicode output needs a stream reconfigure or it raises
`UnicodeEncodeError`.

The `C:\Users\Hp\anaconda3\envs\more_env\python.exe` (torch 2.5.1) and RTX 4060
Laptop 8 GB documented here previously are from the machine the arithmetic POC was
developed on; neither exists on this machine (corrected in T-L0.0). Arithmetic
runs recorded in `runs/` and `README.md` were produced on that machine and are
not re-run.

Quick checks, cheapest first:

```bash
"C:/Users/vedan/anaconda3/python.exe" code/smoke_test.py
```

```bash
"C:/Users/vedan/anaconda3/python.exe" code/verify_pipeline.py
```

```bash
"C:/Users/vedan/anaconda3/python.exe" code/audit_leakage.py
```
...
1. **Scale.** 3.2 M parameters, `d_model = 256`, one recursive block, one
   RTX 4060 Laptop (the machine the arithmetic matrix was run on — the current
   machine is the RTX 3050 of §9; the arithmetic runs are not re-run).
   Whatever MoRE's combination buys may only appear at a scale this study
   cannot reach.
