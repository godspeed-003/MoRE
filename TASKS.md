...
- [x] **T6.2 Authoritative routing accuracy** (§8.2) — one computation, shared
  ...
  **DONE.** There were two independent implementations of the routing accuracy:
  ...
  tolerance cannot be violated. `evaluate_paper_metrics` is retained but marked
  UNUSED in-source so the next agent fixes the copy that runs.
...
- [ ] **T7.2 Correctness suite** (Step 10). Collect the Gate 1–5 assertions
...
- [ ] **T8.0b The routing-supervision decision** — also a prerequisite for T8.2.
...
- [ ] **T8.1 Smoke calibration.** 1–2 epochs on each of MoE / MoR / MoRE;
...
- [ ] **T8.2 Freeze `code/canonical_spec.json`.** Fill the currently-null
...
- [ ] **T9.1 Primary comparison** (§11.1): MoE / MoR / MoRE × seeds
...
- [ ] **T9.2 Parameter-matched MoR** (§11.2): `|P_MoR − P_MoRE| / P_MoRE < 0.05`.
  **Verify:** recorded parameter counts satisfy the bound.
  *Measured on the old code:* MoE 3,724,055 = MoRE 3,724,055, MoR 567,569 →
  **84.8%** mismatch, far outside 5%.

- [ ] **T9.3 Report mean ± std** for best/final val loss, avg depth, normalized
...
- [ ] **T10.B** Adaptive depth vs fixed depth.
- [ ] **T10.C** One vs two recursive blocks — **vary only `num_blocks`**.
  *The old 1-vs-2-block headline varied three factors at once (blocks 1→2,
  max_depth 7→5, `step_routing` 0.0→0.5) and is not interpretable.*
- [ ] **T10.D** Router noise: none vs fixed-annealed vs trainable.
- [ ] **T10.E** Routing supervision off vs on.
- [ ] **T10.F** Dense vs Top-1 sparse routing.
  **Verify (all):** each ablation run differs from canonical in exactly one
  documented field, stated in the table caption.

- [ ] **T10.G** Supplementary sweeps (§13): batch size, capacity/model size.
  Explicitly labelled supplementary; never merged into the headline.

---

## Phase 11 — Export and Report  (`plan.md` §14, §15) — Step 16, 17

- [ ] **T11.1 `code/export_results.py`.** Filter
...
- [ ] **T11.2 `results.csv` / `results.json` / `results.md`** with the 19
...
- [ ] **T11.3 Rewrite `README.md`.** §2–§3 still publish invalidated results,
